...
import { callGeminiWithThinking } from "@/lib/gemini";
...
export const Chatbot = ({ plan, setPlan, isShoppingListOpen, setIsShoppingListOpen, thoughtSteps, setThoughtSteps }: ChatbotProps) => {
  ...
  const addThoughtStep = (thought: string): string => {
  ...
  const updateThoughtStep = (id: string, status: 'pending' | 'active' | 'completed') => {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
        ...
        const { functionCalls, finalAnswer } = await callGeminiWithThinking(
            apiKey,
            history,
            (thought: string) => {
                // Handle streaming thoughts
                if (currentThoughtId) {
                    updateThoughtStep(currentThoughtId, 'completed');
                }
                currentThoughtId = addThoughtStep(thought);
            },
            (answer: string) => {
                // Handle streaming answer
                botResponseText += answer;
                // Update the last message in real-time if needed
            }
        );

        // Complete the last thought step
        if (currentThoughtId) {
            updateThoughtStep(currentThoughtId, 'completed');
        }
        ...
        const botMessage: Message = { 
            id: Date.now() + 1, 
            text: finalAnswer || botResponseText, 
            sender: "bot" 
        };
        ...
        const errorStep: ThoughtStep = {
            ...
            status: 'completed',
            details: `An error occurred while processing your request: ${errorMessage}`
        };
        setThoughtSteps(prev => [...prev, errorStep]);
    } finally {
        setIsThinking(false);
    }
  };
